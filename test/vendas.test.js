// Headless UI test for vendas.html — the sales/event pipeline: clientes,
// the venda pipeline stages, line items, and the payment ledger that
// auto-posts into Financeiro.
//
//   node test/vendas.test.js             # exits non-zero on any failure
...
// relational state (clientes/vendas/venda_itens/venda_pagamentos/
...
const SHOTS = fs.mkdtempSync(path.join(os.tmpdir(), 'vendas-test-'));
...
function serve() {
  return new Promise(resolve => {
    const server = http.createServer((req, res) => {
      ...
      const file = path.join(WEB_DIR, rel || 'vendas.html');
...
function vendaTotals(vendaId) {
  const items = state.itens.filter(i => i.venda_id === vendaId);
  const pays = state.pagamentos.filter(p => p.venda_id === vendaId);
...
(async () => {
  ...
  const PAGE = 'http://127.0.0.1:' + server.address().port + '/vendas.html';
  ...
  await ctx.route('**/functions/v1/checklist-api', async route => {
    ...
    if (body.action === 'clientes') {
    ...
    } else if (body.action === 'vendas') {
      resp = { vendas: state.vendas.map(v => ({ ...withTotals(v), cliente: clienteEmbed(v.cliente_id) })),
        cost_error: null, paid_error: null };
    } else if (body.action === 'venda_detail') {
      const v = state.vendas.find(x => x.id === body.id);
      if (!v) { resp = { found: false, id: body.id }; }
      else {
        const t = withTotals(v);
        resp = {
          found: true,
          venda: { ...v, cliente: clienteEmbed(v.cliente_id) },
          itens: state.itens.filter(i => i.venda_id === v.id),
          pagamentos: state.pagamentos.filter(p => p.venda_id === v.id)
    ...
    } else if (body.action === 'venda_create') {
      const v = {
      ...
      state.vendas.push(v);
      resp = { ok: true, venda: { ...v, cliente: clienteEmbed(v.cliente_id) } };
    } else if (body.action === 'venda_update') {
      const v = state.vendas.find(x => x.id === body.id);
      if (!v) { resp = { error: 'unknown venda' }; }
      else {
    ...
    } else if (body.action === 'venda_delete') {
      const v = state.vendas.find(x => x.id === body.id);
      const items = state.itens.filter(i => i.venda_id === body.id);
      const pays = state.pagamentos.filter(p => p.venda_id === body.id);
      const t = withTotals(v);
      if (!body.force && (items.length > 0 || pays.length > 0)) {
        ...
        state.itens = state.itens.filter(i => i.venda_id !== body.id);
        state.pagamentos = state.pagamentos.filter(p => p.venda_id !== body.id);
        state.lancamentos.forEach(l => {
          if (l.venda_id === body.id) l.venda_id = null;
        });
        state.vendas = state.vendas.filter(x => x.id !== body.id);
        resp = { ok: true, id: body.id, items_removed: items.length, payments_removed: pays.length };
      }
    } else if (body.action === 'venda_item_add') {
      const it = {
        id: uid('I'), venda_id: body.venda_id, tipo: body.tipo, description: body.description,
        quantity: body.quantity, unit_cost: body.unit_cost || 0, unit_price: body.unit_price || 0,
        created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      };
      state.itens.push(it);
      resp = { ok: true, item: it };
    } else if (body.action === 'venda_item_update') {
      const it = state.itens.find(x => x.id === body.id);
      ['tipo', 'description', 'quantity', 'unit_cost', 'unit_price'].forEach(k => {
        if (k in body) it[k] = body[k];
      });
      resp = { ok: true, item: it };
    } else if (body.action === 'venda_item_delete') {
      const it = state.itens.find(x => x.id === body.id);
      state.itens = state.itens.filter(x => x.id !== body.id);
      resp = { ok: true, id: body.id, venda_id: it ? it.venda_id : null };
    } else if (body.action === 'venda_payment_confirm') {
      const v = state.vendas.find(x => x.id === body.venda_id);
      const pay = {
        id: uid('P'), venda_id: body.venda_id, amount: body.amount,
      ...
      const lanc = {
        id: uid('L'), tipo: 'receita', amount: body.amount, categoria: 'venda',
        description: 'Pagamento — ' + v.name, occurred_at: pay.received_at,
        venda_id: body.venda_id, venda_pagamento_id: pay.id, stock_movement_id: null,
        source: 'venda_payment', note: null,
        created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      };
      state.lancamentos.push(lanc);
      const t = withTotals(v);
      resp = { ok: true, payment: pay, lancamento: lanc, total_paid: t.total_paid, balance_due: t.balance_due };
    } else if (body.action === 'venda_payment_update') {
      ...
      if (changed) {
        const lanc = state.lancamentos.find(l => l.venda_pagamento_id === pay.id);
        if (lanc) {
      ...
      const v = state.vendas.find(x => x.id === pay.venda_id);
      const t = withTotals(v);
      resp = { ok: true, payment: pay, lancamento_updated, total_paid: t.total_paid, balance_due: t.balance_due };
    } else if (body.action === 'venda_payment_delete') {
      const pay = state.pagamentos.find(x => x.id === body.id);
      const vendaId = pay.venda_id;
      state.pagamentos = state.pagamentos.filter(x => x.id !== body.id);
      state.lancamentos.forEach(l => {
        if (l.venda_pagamento_id === body.id) l.venda_pagamento_id = null;
      });
      const v = state.vendas.find(x => x.id === vendaId);
      const t = v ? withTotals(v) : { total_paid: 0, balance_due: 0 };
      resp = { ok: true, id: body.id, venda_id: vendaId, total_paid: t.total_paid, balance_due: t.balance_due };
  ...
  await page.waitForSelector('#new-venda', { timeout: 6000 });
  check('empty state shows the create button', (await page.$('#new-venda')) !== null);

  // --- create a cliente inline via the picker, then a venda ---
  await page.click('#new-venda');
  ...
  await page.waitForSelector('.venda-head', { timeout: 6000 });
  check('a venda created at lead stage renders lead', (await page.textContent('.pill')).includes('Lead'));
  check('one cliente exists', state.clientes.length === 1);
  check('one venda exists', state.vendas.length === 1);
  ...
  for (const [from, to] of [['lead', 'orcamento'], ['orcamento', 'confirmado']]) {
    ...
    await page.waitForFunction(
      (label) => document.querySelector('.venda-head .pill')?.textContent === label,
      to === 'orcamento' ? 'Orçamento' : 'Confirmado', { timeout: 6000 });
  }
  check('status reached confirmado', state.vendas[0].status === 'confirmado');
  ...
  const vendaId = state.vendas[0].id;
  ...
  check('exactly one linked lançamento exists after the first payment',
    state.lancamentos.filter(l => l.venda_id === vendaId).length === 1);
  ...
  check('two payments reach the full 100,00 balance',
    norm(await page.textContent('.totals-grid')).includes('R$ 100,00') &&
    vendaTotals(vendaId).total_paid === 100);
  ...
  await page.waitForSelector('.venda-head', { timeout: 6000 });
  const editedLanc = state.lancamentos.find(l => l.amount === 45 && l.venda_id === vendaId);
  ...
  check('the linked ledger row survives with venda_pagamento_id nulled',
    editedLanc.venda_pagamento_id === null && editedLanc.amount === 45);
  ...
  await page.click('#del-venda');
  await page.waitForSelector('#confirm-ok', { timeout: 6000 });
  await page.click('#confirm-ok');
  await page.waitForSelector('#confirm-ok', { timeout: 6000 }); // the has_content follow-up dialog
  check('the follow-up dialog is shown because the venda has content',
    (await page.textContent('.modal-card p')).includes('itens') ||
    (await page.textContent('.modal-card p')).includes('pagamento'));
  await page.click('#confirm-ok');
  await page.waitForFunction(() => document.getElementById('new-venda') !== null, null, { timeout: 6000 });
  check('the venda is gone from state', state.vendas.length === 0);
  check('both remaining ledger rows survive with venda_id nulled',
    state.lancamentos.every(l => l.venda_id === null) && state.lancamentos.length === 2);

  // --- deep link opens the right sale ---
  // Recreate a venda directly through the UI for the deep-link check.
  await page.click('#new-venda');
  await page.waitForSelector('#nv-cliente-btn', { timeout: 6000 });
  await page.click('#nv-cliente-btn');
  await page.waitForSelector('#cli-q', { timeout: 6000 });
  ...
  await page.fill('#nv-name', 'Venda para deep link');
  await page.click('#nv-ok');
  await page.waitForSelector('.venda-head', { timeout: 6000 });
  const deepId = state.vendas[0].id;
  await page.click('#back');
  await page.waitForSelector('#new-venda', { timeout: 6000 });
  await page.goto(PAGE + '?id=' + deepId);
  await page.waitForSelector('.venda-head', { timeout: 6000 });
  check('the deep link opens the right sale directly',
    (await page.textContent('.venda-head .name')).includes('Venda para deep link'));
  ...
  // confirm the venda's OWN detail sheet still opens cleanly.
  await page.evaluate(async (id) => {
    await window.api('cliente_delete', { id });
  }, clienteId);
  await page.click(`.row[data-id="${deepId}"]`);
  await page.waitForSelector('.venda-head', { timeout: 6000 });
  check('the sale renders "Sem cliente" after its cliente is deleted, no crash',
    (await page.textContent('.cli-block')).includes('Sem cliente'));

  await page.screenshot({ path: path.join(SHOTS, 'venda_detalhe.png'), fullPage: true });
  await browser.close();
  server.close();
